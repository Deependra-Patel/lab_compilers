...
ExpAst * make_boolean(ExpAst * exp) {
	...
	if (exp->type->tag == Base && exp->type->basetype == Int)
		...
		type_str = "Float";
	bool do_it = false;
	if (type == "Identifier" || type == "Index" || type == "IntConst" || type == "FloatConst") {
		do_it = true;
	}
...
void Funcall::generate_code(SymbolTable* st){
	...
	if (calledFunc == "printf") {
		cout << "in printf" << endl;
		gencode("    print_string("+((StringConst *)children[1])->child+");");
		for (int i = 2; i < children.size(); i++) {
			children[i]->generate_code(st);
			if (children[i]->type->tag == Base && children[i]->type->basetype == Int)
				gencode("    print_int("+regs.back()+");");
			else if(children[i]->type->tag == Base && children[i]->type->basetype == Float)
				gencode("    print_float("+regs.back()+");");
			if (i < children.size() -1) 
				gencode("    print_char(' ');");
			else gencode("    print_char('\\n');");
		}
		cout << "out of printf" << endl;
		return;
	}
	if (calledFunc == "print") {
