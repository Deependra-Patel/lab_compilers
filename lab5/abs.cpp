...
#include <algorithm>
using namespace std;
vector<string> regs = {"edx", "ecx", "ebx"};
vector<string> all_regs = {"edx", "ecx", "ebx"};
extern Code code;
...
void save_regs() {
	for (int i = 0; i < all_regs.size() ; i++) {
		gencode("    pushi("+ all_regs[i] +");");
	}
}

void load_regs() {
	for (int i = all_regs.size() - 1; i >= 0; i--) {
